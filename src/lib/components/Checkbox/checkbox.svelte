<script lang="ts">
	...
	export let props: Props = { variant: 'default', checked: false };
	...
	$: _class = `veniz veniz-checkbox  ${variant} ${addClass ?? ''} ${props.addClass ?? ''}`.trim();
	/* class="pl-2 veniz-label cursor-default" */
</script>

<div name="input" class="veniz-checkbox-wrapper">
	<input type="checkbox" class={_class} {checked} on:click />
	{#if $$slots.label}
		<slot name="label" />
	{:else if label}
